...
module.exports = event => new Promise((resolve, reject) => {
    ...
    const getOrCreateStripeCustomer = (user, stripeToken) => {
        ...
        return new Promise((resolve, reject) => {
            stripe_1.default.customers.create({
                email,
    ...
    const chargeStripeCustomer = (amount, description, stripeCustomerId) => new Promise((resolve, reject) => {
        const charge = {
            amount,
            description,
        ...
        if (event.data.phone) {
            const charge = Object.assign({}, charge, {
                receipt_phone: event.data.phone,
            });
        }
        return stripe_1.default.charges.create(charge, (err, charge) => {
            if (err) {
    ...
    return getBasket(basketId)
        ...
        return getOrCreateStripeCustomer(user, stripeToken)
            .then(stripeCustomerId => chargeStripeCustomer(orderTotal, description, stripeCustomerId))
            .then(stripeCustomerId => convertBasketToOrder(Object.assign({}, data, {
            stripeCustomerId,
            orderTotal,
        })))
            .then(({ createOrder }) => {
...
const getOrderTotal = items => items.reduce((sum, item) => sum + item.orderedItem.amount * item.quantity, 0);
const getDescription = items => items
    .map(item => `${item.quantity}x ${item.orderedItem.name} @ $${item.orderedItem.amount} each`)
    .join(', ');
